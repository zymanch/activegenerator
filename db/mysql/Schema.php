...
class Schema extends \ActiveGenerator\db\Schema
{
    ...
    protected function findColumns($table)
    {
        ...
        foreach ($columns as $info) {
            if ($this->db->getAttribute(\PDO::ATTR_CASE) !== \PDO::CASE_LOWER) {
    ...
    {
        $prepare = $this->db->prepare('SHOW CREATE TABLE ' . $this->quoteTableName($table->fullName));
        $prepare->execute();
        $row = $prepare->fetch();
...
SQL;
        ...
        try {
            ...
            $prepare->execute([':schema' => $table->schemaName, ':tableName' => $table->name, ':tableName1' => $table->name]);
